...
void ControllerComponent::Initialize(const SceneContext& sceneContext)
{
	if(!m_IsInitialized)
	{
		...
		//m_ControllerDesc.setToDefault();
		//m_ControllerDesc.radius = m_Radius;
		//m_ControllerDesc.height = m_Height;
		//m_ControllerDesc.climbingMode = m_ClimbingMode;
		//m_ControllerDesc.upDirection = PxVec3(0, 1, 0);
		//m_ControllerDesc.contactOffset = 0.1f;
		//m_ControllerDesc.material = m_pMaterial;
		m_ControllerDesc.position = PxExtendedVec3( m_pGameObject->GetTransform()->GetPosition().x , m_pGameObject->GetTransform()->GetPosition().y , m_pGameObject->GetTransform()->GetPosition().z);
		...
		m_pController->getActor()->setName(m_Name.c_str());
