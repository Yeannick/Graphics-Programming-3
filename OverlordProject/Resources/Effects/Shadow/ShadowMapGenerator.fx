...
float4 ShadowMapVS_Skinned(float3 position:POSITION, float4 BoneIndices : BLENDINDICES, float4 BoneWeights : BLENDWEIGHTS) : SV_POSITION
{
	...
	for (int i{}; i < 4; ++i)
	{
		...
		if (index > -1)
		{
			transformedPosition += mul(originalPosition, gBones[index]) * BoneWeights[i];
		}
	}
	...
	//float4 pos = mul(skinnedPos, mul(gWorld, gLightViewProj));
	float4 pos = mul(transformedPosition, mul(gWorld, gLightViewProj));
