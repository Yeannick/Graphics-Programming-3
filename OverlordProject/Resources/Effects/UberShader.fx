...
float3 CalculateSpecularBlinn(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	float4 specColor = gColorSpecular;
	float3 halfVector = normalize(viewDirection + gLightDirection);
	float specularStrength = abs(dot(halfVector, normal));
	
	float specularValue = pow(specularStrength, gShininess);

	if (gUseTextureSpecularIntensity)
		specColor *= gTextureSpecularIntensity.Sample(gTextureSampler, texCoord).r;

	return saturate(specColor * specularValue);

	/*float3 R = -reflect(gLightDirection, normal);
	float RdotV = saturate(dot(R, viewDirection));
	if (gUseTextureSpecularIntensity)
	{
		float3 sampledSpec = gTextureSpecularIntensity.Sample(gTextureSampler, texCoord).rgb;
		return (gColorSpecular + sampledSpec) * pow(RdotV, gShininess);
	}
	return gColorSpecular * pow(RdotV, gShininess);*/
}
...
float3 CalculateSpecular(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	float3 diffuseColor = float3(0.f, 0.f, 0.f);
		...
		if (gUseSpecularBlinn && gUseSpecularPhong)
		{
			...
			diffuseColor = CalculateSpecularPhong(viewDirection, normal, texCoord);
			return specColor + diffuseColor;
		}
		if (gUseSpecularPhong &! gUseSpecularBlinn)
		{
			diffuseColor = CalculateSpecularPhong(viewDirection, normal, texCoord);
			return diffuseColor;
		}
...
float3 CalculateDiffuse(float3 normal, float2 texCoord)
{
	...
	{
		diffColor = diffColor * float3(gTextureDiffuse.Sample(gTextureSampler, texCoord).xyz) * diffuseLightValue;
		return diffColor;
	}
	//diffColor = float3(gTextureDiffuse.Sample(gTextureSampler,texCoord).xyz);
	diffColor = diffColor * diffuseLightValue;
	return diffColor;
}
...
float3 CalculateFresnelFalloff(float3 normal, float3 viewDirection, float3 environmentColor)
{
	...
	{
		return saturate(fresnel * fresnelMask * environmentColor);
	}
	return saturate(fresnel * fresnelMask * gColorFresnel);
...
float3 CalculateEnvironment(float3 viewDirection, float3 normal)
{
	...
	if (gUseEnvironmentMapping)
	{
		float3 colorReflect = gCubeEnvironment.Sample(gTextureSampler, reflectedVector) ;
		float3 colorRefract = gCubeEnvironment.Sample(gTextureSampler, refractedVector);
		float3 colorCube = (colorReflect * gReflectionStrength) + (colorRefract * gRefractionStrength);
		return colorCube ;
	}
	return float3(0.f, 0.f, 0.f);
}
...
float4 MainPS(VS_Output input) : SV_TARGET{
	...
	float3 finalColor = saturate(diffColor + specColor + environmentColor + ambientColor);
